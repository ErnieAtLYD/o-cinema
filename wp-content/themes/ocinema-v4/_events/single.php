...
if (isset($evtinfo)) {
	...
	if ( isset($json) ) {
		...
		$xml_media_embed = $show['AdditionalMedia']['Media'][0];
	}
}
...
?>
	...
	<div class="body container">
			...
			<div class="purchase-tix thumbnail">
				...
				if (isset($show['CurrentShowings']['Showing'])) {
					...
					foreach ( $showing_arr as $agile_event ){
						...
						$date = $dowMap[date("w", $timestamp)] . date(", M jS", $timestamp);
						if ( ! isSet($sortedData[$date]) ) { //first entry of that day
							$sortedData[$date] = array($agile_event);
						} else { 
							//just push current element onto existing array
							$sortedData[$date][] = $agile_event;
						}
					}

					foreach ( $sortedData as $date_s => $date_elems ) {
					...
					if ( count($meta) == 0 ) {
						...
						foreach($meta as $key) {
							echo '<tr><td>';
							if (get_post_meta($post->ID, 'ticketurl', true)) { 
								echo '<a href="' . get_post_meta($post->ID, 'ticketurl', true) . '" target="_blank">'; }
							echo $key->meta_value;
							if ( get_post_meta($post->ID, 'ticketurl', true) ) {
								echo '</a>'; 
							}
					...
					if (get_post_meta($post->ID, 'ticketurl', true)) : 
						...
						echo ' btn-block" href="';
						echo get_post_meta($post->ID, 'ticketurl', true);
		...
		<span class="span8">
			...
			<?php
                        if ( wp_oembed_get(get_post_meta($post->ID, "trailer", true) )) {
				echo '<div style="clear:both;" id="trailer">';
				echo wp_oembed_get( get_post_meta($post->ID, "trailer", true) );
				echo '</div>';
			...
			<div class="row">
				<div class="span5">
<?php
/* echo tribe_get_start_date(null, FALSE, 'd G:i e') . '<br>';
 echo tribe_get_end_date(null, FALSE, 'd G:i e') . '<br>';
 echo date('d G:i e', time()) . '<br>';
 print_r( tribe_get_end_date(null, FALSE, 'U') < time() ); */
?>
